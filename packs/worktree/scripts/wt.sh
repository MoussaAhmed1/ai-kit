...
# Generate docker-compose.worktree.yml override with port offsets + container name isolation
generate_docker_compose_override() {
    ...
    cn_data=$(parse_docker_compose_container_names "$compose_file")

    [[ -z "$ports_data" && -z "$cn_data" ]] && return 1
...
# Port offsets and container name isolation for worktree
services:
HEADER

    # Collect unique service names from both datasets
    local services=()
    local seen=""
    for dataset in "$ports_data" "$cn_data"; do
        [[ -z "$dataset" ]] && continue
    ...
    for svc in "${services[@]}"; do
        ...
        # Container name override (only for services with explicit container_name)
        if [[ -n "$cn_data" ]]; then
            local cn
            cn=$(printf '%s\n' "$cn_data" | grep "^${svc}:" | head -1 | cut -d: -f2-)
            if [[ -n "$cn" ]]; then
                printf '    container_name: %s_%s\n' "$cn" "$branch_slug" >> "$tmpfile"
            fi
        fi
        ...
        if [[ -n "$ports_data" ]]; then
            ...
            if [[ -n "$svc_ports" ]]; then
                printf '    ports:\n' >> "$tmpfile"
                while IFS=: read -r _s host_port container_port; do
...
setup_docker_isolation() {
    ...
    local override_file
    override_file=$(generate_docker_compose_override "$compose_full" "$offset" "$branch_slug") || {
        warn "No ports or container names found in compose file, skipping override"
        return 0
    }
...
cmd_remove() {
    ...
    # Stop Docker containers if compose setup exists (check root and subdirectories)
    if command -v docker &>/dev/null; then
        while IFS= read -r -d '' env_file; do
            ...
            if [[ -n "$compose_file_val" ]]; then
                ...
                info "Stopping Docker containers ($(basename "$env_dir"))..."
                (cd "$env_dir" && docker compose down 2>/dev/null) || warn "Could not stop containers (may not be running)"
